...
# data for trap location information ("blurred" to protect currently operating machinery), and counts
# of male and female Aedes aegypti collected at those traps, with collection dates
# are available online at:
# (a) link 1
# (b) link 2
# (c) dataframe "df.csv" that results from example code below, which can be used directly in PART 3 of this R script:
#     xxxx


# Where the code below indicates setting the directory to "~/.../", this indicates
# that the user should point the code to wherever the data are stored, either locally or online.

# A file structure that can be used if all of the data files are stored locally (i.e. on a user's computer,
# github account, Dropbox folder or other cloud-based folder, or other pathway that can be accessed by the user directly)
# is the following:
...
# load PPT (precipiation) layers 
# these are available on FigShare in a .zip file, but must be unzipped and stored elsewhere for use
#examples of rasters are given in this github repository, but the full set of layers must be accessible for the analyses
setwd("~/.../aedes_precip/")
...
calPPT <- function(daysAgo, dayRange,ooo){
  ...
  for(i in 1:length(trapDay_all)){
    ...
    selDays <- seq.Date (from= startDay, to= endDay, by=1)  #selects days in a sequence from a given start date
